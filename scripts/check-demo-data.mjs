...
try {
  const users = await sql`
    ...
    from "user" where email like '%@kintwadi.demo' order by email`;
  ...
  const [demo] = await sql`
    ...
    where u.email = 'maria@kintwadi.demo' limit 1`;
  if (!demo) {
    ...
    if (counts[0].chunks === 0) console.log('  ⚠ rag_chunk is empty — POST /api/ingest (as platform admin) so Ask Kintwadi has vectors.');
  }
} catch (e) {
