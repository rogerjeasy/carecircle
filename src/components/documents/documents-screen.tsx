...
export function DocumentsScreen({ initial }: DocumentsScreenProps) {
  ...
  const categoryLabel = React.useCallback(
    (c: DocCategory) => t(`categories.${c}` as "categories.medical"),
    [t]
  );
  const visible = React.useMemo(
    () =>
      docs.filter((d) => {
        ...
        return d.title.toLowerCase().includes(q) || categoryLabel(d.category).toLowerCase().includes(q);
      }),
    [docs, category, q, role, categoryLabel]
  );

  const openFile = (doc: DocumentItem) => {
    if (doc.downloadUrl) window.open(doc.downloadUrl, "_blank", "noopener,noreferrer");
    else toast.error(t("toasts.fileUnavailable"));
  };
  ...
  const handleAction = (doc: DocumentItem, action: DocAction) => {
    switch (action) {
      ...
      case "emergency": {
        ...
        toast(doc.onEmergencyCard ? t("toasts.removedEmergency") : t("toasts.addedEmergency"));
        void toggleEmergencyCard(doc.id).then((res) => {
          if (!res.ok) {
            setDocs(prev);
            toast.error(res.error ?? t("toasts.emergencyError"));
          }
        });
        break;
      }
  ...
  const confirmRename = async (title: string) => {
    ...
    if (res.ok) toast.success(t("toasts.renamed"));
    else {
      setDocs(prev);
      toast.error(res.error ?? t("toasts.renameError"));
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    if (res.ok) {
      toast(t("toasts.deleted", { title: target.title }));
      router.refresh();
    } else {
      setDocs(prev);
      toast.error(res.error ?? t("toasts.deleteError"));
    }
  };
  ...
  const handleUpload = async (files: File[], meta: UploadMeta): Promise<DocumentItem[]> => {
    ...
    for (const file of files) {
      ...
      else toast.error(res.error ?? t("toasts.uploadFileError", { name: file.name }));
    }
    return created;
  };
  ...
  const handleUploaded = (newDocs: DocumentItem[]) => {
    if (newDocs.length > 0) {
      ...
      toast.success(t("toasts.uploaded", { count: newDocs.length }));
    }
    setUploadOpen(false);
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        ...
        <Select value={category} onValueChange={(v) => setCategory(v as DocCategory | "all")}>
          <SelectTrigger className="h-11 w-full sm:w-52" aria-label={t("search.filterAria")}>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {CATEGORY_FILTERS.map((c) => (
              <SelectItem key={c.value} value={c.value}>
                {c.value === "all" ? t("categoryFilters.all") : categoryLabel(c.value)}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      <AlertDialog open={deleting !== null} onOpenChange={(o) => !o && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("delete.description", { title: deleting?.title ?? "" })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
    ...
    <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed px-6 py-16 text-center">
      ...
      <div>
        <p className="text-base font-semibold">
          {variant === "empty" ? t("empty.emptyTitle") : t("empty.noResultsTitle")}
        </p>
        <p className="mt-1 text-sm text-muted-foreground">
          {variant === "empty" ? t("empty.emptySubtitle") : t("empty.noResultsSubtitle")}
        </p>
      </div>
