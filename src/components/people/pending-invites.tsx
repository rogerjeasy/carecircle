...
export function PendingInvites({ invites, canManage, onResend, onRevoke }: PendingInvitesProps) {
  ...
  return (
    <section aria-label={t("invites.aria")} className="space-y-3">
      <div className="flex items-center gap-2">
        <Mail className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
        <h2 className="text-sm font-semibold">{t("invites.heading")}</h2>
              ...
              <div className="flex min-w-0 flex-1 items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    {t("invites.sentOn", {
                      date: invite.sentAt.toLocaleDateString(locale, { year: "numeric", month: "short", day: "numeric" }),
                    })}
                  </p>
                </div>
              </div>
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                <GatedControl canManage={canManage}>
                  <Button variant="outline" size="sm" className="h-8" onClick={() => onResend(invite)}>
                    ...
                    <span className="ml-1 hidden sm:inline">{t("invites.resend")}</span>
                  </Button>
                </GatedControl>
                <GatedControl canManage={canManage}>
                  <Button
                  ...
                  >
                    ...
                    <span className="ml-1 hidden sm:inline">{t("invites.revoke")}</span>
                  </Button>
                </GatedControl>
              </div>
            </div>
