...
export function DigestScreen({
  ...
  // Human label localized via native Intl (project convention); dateStr stays a machine key.
  const dateLabel =
    offset === 0
      ? t("today")
      : offset === -1
  ...
  const giveFeedback = (value: "up" | "down") => {
    ...
    if (next) toast(t("feedbackThanks"));
    ...
    <div className="space-y-6">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm">
              ...
              <span className="ml-1">{t("settingsBtn")}</span>
            </Button>
          </PopoverTrigger>
          <PopoverContent align="end" className="w-72">
            <p className="text-sm font-semibold">{t("whenWritten")}</p>
            <p className="mt-3 flex items-center gap-1.5 text-sm">
              <Clock className="h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
              <span className="font-medium">{t("cadence")}</span>
              <span className="text-muted-foreground">{t("atTime", { time: DIGEST_SETTINGS.time })}</span>
            </p>
      ...
      <Card>
        <CardContent className="flex items-center justify-between gap-3 p-3 sm:p-4">
          <Button variant="outline" size="icon" className="h-9 w-9 shrink-0" onClick={() => setOffset((o) => o - 1)} aria-label={t("prevDay")}>
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <div className="min-w-0 text-center">
            <p className="truncate text-sm font-semibold">{dateLabel}</p>
            <p className="truncate text-xs text-muted-foreground">{date.toLocaleDateString(locale, { weekday: "long", month: "long", day: "numeric" })}</p>
          </div>
          <Button
            variant="outline"
            size="icon"
        ...
        {loading || generating ? (
          ...
          <>
              ...
              <div className="flex items-center justify-between gap-2 rounded-xl border bg-primary/5 px-3 py-2 text-sm">
                <span className="flex min-w-0 items-center gap-1.5 text-muted-foreground">
                  ...
                  <span className="truncate">
                    {showOriginal ? t("showingOriginal") : t("translatedTo", { lang: langMeta.nativeLabel })}
                  </span>
                </span>
                <Button variant="ghost" size="sm" className="shrink-0" onClick={() => setShowOriginal((v) => !v)}>
                  {showOriginal ? t("readIn", { lang: langMeta.nativeLabel }) : t("showOriginal")}
                </Button>
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => toast.success(t("shared"))}>
              ...
              <Button variant="outline" size="sm" onClick={toggleListen} aria-pressed={speaking}>
                ...
                <span className="ml-1">{speaking ? t("stop") : t("listen")}</span>
              </Button>
              {canGenerate && (
              ...
              <div className="ml-auto flex items-center gap-1">
                <span className="mr-1 text-xs text-muted-foreground">{t("helpful")}</span>
                <Button
                  variant="ghost"
                  size="icon"
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed px-6 py-16 text-center">
      ...
      <div>
        <p className="text-base font-semibold">{t("emptyTitle", { label })}</p>
        <p className="mt-1 text-sm text-muted-foreground">
          {canGenerate ? t("emptyCanGenerate") : t("emptyReadOnly")}
        </p>
      </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          {onShowYesterday && (
            <Button variant="outline" onClick={onShowYesterday}>
              <ChevronLeft className="h-4 w-4" />
              <span className="ml-1">{t("readYesterday")}</span>
            </Button>
          )}
        </div>
      )}
