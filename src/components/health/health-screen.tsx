...
import { MetricGridSkeleton, FocusChartSkeleton } from "./health-skeletons";
import { LogReadingModal } from "./log-reading-modal";
import { GatedControl } from "./gated-control";
import { buildReadings, CARE_RECIPIENT, DEFAULT_THRESHOLDS, METRIC_ORDER } from "./data";
...
import type { MetricKey, RangeKey, Reading } from "./types";

const RANGES: RangeKey[] = ["7d", "30d", "90d"];

/** The Health screen: metric cards, a focus chart, insights, and a log-reading flow. */
export function HealthScreen() {
  const { role } = useAppShell();
  const canLog = canLogReadings(role);

  const [now] = React.useState(() => new Date());
  const [readings, setReadings] = React.useState<Reading[]>(() => buildReadings(now));
  ...
  const handleLog = (values: LogValues) => {
    const reading = logValuesToReading(values, `reading-new-${newIdRef.current++}`);
    setReadings((prev) => [reading, ...prev]);
    setFocus(reading.metric);
    setLogOpen(false);
    toast.success("Reading saved", { description: "Added to the chart." });
    // A second, gentle confirmation that it reached the family feed.
    setTimeout(() => toast("Posted to the timeline"), 350);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-muted-foreground">{CARE_RECIPIENT.name}&apos;s vitals at a glance.</p>
        </div>
      ...
      {logOpen && (
        <LogReadingModal
          ...
          initialValues={defaultLogValues(now, focus)}
          onSubmit={handleLog}
        />
      )}
    </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed px-6 py-20 text-center">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Log the first vital to start tracking trends.</p>
      </div>
      {canLog && (
